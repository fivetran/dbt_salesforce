...
- Number of materialized models¹: 27
...
vars:
  ...
  salesforce__campaign_history_enabled: true  # False by default. Only use if you have history mode enabled and wish to view the full historical record of all your synced campaign fields, particularly in the daily campaign history model.
...
vars:
    ...
    campaign_history_start_date: 'YYYY-MM-DD' # The first date in campaign history you wish to pull records from, filtering on `_fivetran_start`.
...
vars:
  ...
  salesforce__campaign_pass_through_columns: 
    - name: "salesforce__campaign_field"
      alias: "campaign_field_x"
```
